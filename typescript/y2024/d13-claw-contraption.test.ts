...
const PositionUtils = {
	add: ([aY, aX]: Pos, [bY, bX]: Pos): Pos => [aY + bY, aX + bX],
	toString: ([y, x]: Pos, separator = ",") => `${y}${separator}${x}`,
};
...
const file = process(`Button A: X+24, Y+19
Button B: X+32, Y+86
Prize: X=1072, Y=2062
...
describe("part one", () => {
	it("runs", () => {
		expect(partOne(example)).toStrictEqual(480);
		expect(partOne(file)).toStrictEqual(29598);
	});
});

describe("part two", () => {
	it("runs", () => {
		expect(partOne(file)).toStrictEqual(93217456941970);
	});
});
